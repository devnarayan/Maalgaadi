...
?>
            ...
            $sql = "select booking.*, booking_short.distance_to_customer as to_customer,vehicle.registration_no, vehicle.name as vname  ,customer.customer_name,customer.customer_organization as customer_organize, booking_short.trip_time,booking_short.trip_distance,booking_short.payment_type,booking_short.payment,booking_short.total_payment_amount,booking_short.balance,booking_short.booking_type from booking inner join customer on customer.id=booking.customer_id left join booking_short on booking.id=booking_short.booking_id left join vehicle on vehicle.id=booking.vehicle_id where 1 $search1 $search2 $search3 $search4 group by booking_id";
            ...
            while($row=$objConnect->fetch_assoc()){
                ...
                <tr>
                    ...
                    <td><?php echo $row['vname']."<br>".$row['registration_no'];?></td>
                    <td><?php if($row['total_payment_amount']==""){echo $row['total_fare']." Estimated";$total_estimate=$total_estimate+$row['total_fare'];}else{echo $row['total_payment_amount'];$total_amount=$total_amount+$row['total_payment_amount'];}?></td>
                    <td><?php echo $row['payment_type'];?></td>
                    <td><?php echo $row['payment'];$total_payment=$total_payment+$row['payment'];?></td>
                    <td><?php echo $row['balance'];$total_balance=$total_balance+$row['balance'];?></td>
                    <td><?php if($row['status']==7){echo "Completed";}
                    elseif($row['status']==99){echo "Canceled";}
                    elseif($row['status']>=2){echo "In Transit";}
                    elseif($row['status']>=-3){echo "Pending";}?></td>
                </tr>
            <?php 
            }
            ?>
