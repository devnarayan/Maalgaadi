...
if((isset($_POST['customer_id']))&&(!empty($_POST['customer_id']))){ $search1= " and customer.id like '".$_POST['customer_id']."'"; }
...
$total_discount = 0;$total_cahreceive = 0;$da=0;$ca=0;
...
//$getOpening = "select DISTINCT CASE WHEN cashreceive_date IN (DATE(pickup_date)) THEN cash_amount ELSE '0.00' END AS cash_amount,DATE(pickup_date) as pickDate,CASE WHEN discount_date IN (DATE(pickup_date)) THEN discount_amount ELSE '0.00' END AS discount_amount,booking.pickup_date from booking inner join customer on customer.id=booking.customer_id left join booking_short on booking.id=booking_short.booking_id left join vehicle on vehicle.id=booking.vehicle_id left join discount on discount.customer_id=customer.id and DATE(discount.discount_date) = DATE(booking.pickup_date) left join cashreceive on cashreceive.customer_id=customer.id and DATE(cashreceive.cashreceive_date) = DATE(booking.pickup_date) where 1 $search1 $openingDateStart $openingDateEnd group by pickDate";
	 ...
	 $getOpening = "SELECT distinct date(pickup_date) as pickup_date FROM booking where 1 $search99 $openingDateStart $openingDateEnd group by pickup_date UNION SELECT distinct date(cashreceive_date) FROM cashreceive where 1 $customerFilter $cashFilter1 $cashFilter2 UNION SELECT distinct date(discount_date) FROM discount where 1 $customerFilter $disountFilter1 $disountFilter2";
		 ...
		 //  echo $total_balance.'D<br/>';
		  ...
		  //get discount
	   		$sql112 = "select DISTINCT CASE WHEN discount_date IN (DATE(discount_date)) THEN discount_amount ELSE '0.00' END AS discount_amount,discount.discount_date,discount.narration as dn from discount Left join customer on discount.customer_id=customer.id where 1 $search1 and discount_date >= '".$date11."' and discount_date <= '".$date12."' group by discount_date";
...
$newGetDate = "SELECT distinct date(pickup_date) as pickup_date FROM booking where 1 $search99 $search2 $search3 group by pickup_date UNION SELECT distinct date(cashreceive_date) FROM cashreceive where 1 $search99 $searchC $searchD UNION SELECT distinct date(discount_date) FROM discount where 1 $search99 $searchC1 $searchD1 ORDER BY  `pickup_date` ASC ";
	   ...
	   //get discount
			...
			if($row112['discount_date'] != '' || $row112['discount_date'] != null)
			  ...
			  <tr>
				...
				<td colspan="4"><b>Discount Given ( <?php echo $row112['dn'];?> ) </b></td>
       		...
       		if($row1['cashreceive_date'] != '' || $row1['cashreceive_date'] != null)
				...
				?>
				  <tr>
					...
					<td colspan="4"><b>Cash received ( <?php echo $row1['cn'];?> )</b></td>
  ...
  <tr> 
    ...
    <td><?php if($row325['total_payment_amount']=="")
				{echo $row325['total_fare']." Estimated";}
				else
				{echo $row325['total_payment_amount'];}?></td>
